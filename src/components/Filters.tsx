    ...
    <div className="border-b border-ink/[.08]">
      <button type="button" onClick={() => setOpen(o => !o)} aria-expanded={open} className="w-full flex items-center justify-between py-3.5 text-left group">
        <span className="label-caps text-ink flex items-center gap-2">
          {title}
          {badge ? <span className="inline-flex items-center justify-center min-w-[1.1rem] h-[1.1rem] rounded-full bg-ink text-paper text-[9px] px-1 tabular-nums">{badge}</span> : null}
        </span>
        <span className={`w-6 h-6 rounded-full ring-1 ring-ink/10 flex items-center justify-center text-fog group-hover:ring-steel group-hover:text-steel transition-all duration-300 ${open ? 'rotate-180' : ''}`}>
