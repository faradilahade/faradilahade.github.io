...
import { IconPlus, IconLayers, IconEdit, IconLanguage, IconArrowRight } from '../../components/Icons'
...
export default function Overview({ projects, onNew, onOpenProjects, onEdit }: Props) {
  const published = projects.filter(p => p.published).length
  const drafts = projects.length - published
  const featured = projects.filter(p => p.featured).length
  const tools = new Set(projects.flatMap(p => p.tools.map(x => x.toLowerCase()))).size
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          ['Drafts', drafts, 'not visible yet'],
          ['Featured', featured, 'shown first'],
          ['Tools', tools, 'distinct across projects'],
        ].map(([k, v, sub]) => (
          <div key={String(k)} className={tile}>
            <p className="label-caps">{k}</p>
            <p className="mt-2 font-display text-fl-3xl leading-none">{v}</p>
      ...
      <div className="grid lg:grid-cols-[1.4fr_1fr] gap-6 items-start">
        <div className={tile}>
          <div className="flex items-center justify-between gap-4">
            <h3 className="font-bold uppercase tracking-tight text-fl-sm">Recently updated</h3>
