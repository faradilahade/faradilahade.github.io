import { useEffect, useMemo, useRef, useState, ChangeEvent, DragEvent, ReactNode } from 'react'
...
import { IconUpload, IconClose, IconEye, IconLanguage, IconArrowLeft, IconArrowRight, IconCheck, IconExternal } from '../../components/Icons'
import {
...
const field = 'w-full border border-line rounded-lg px-3 py-2 bg-white text-ink focus:border-steel outline-none transition-colors'
const label = 'label-caps block mb-1.5'
...
function Dropzone({ onFiles, accept, multiple, children, busy }: { onFiles: (files: File[]) => void; accept?: string; multiple?: boolean; children: ReactNode; busy?: boolean }) {
...
export default function ProjectEditor({ draft, onChange, onSave, onCancel, saving, previewHref }: Props) {
  ...
  // ---- content toolbar ---------------------------------------------
  function format(kind: 'h2' | 'bullet' | 'number' | 'quote' | 'bold' | 'italic' | 'link') {
  ...
  const preview = (
    <div className="bg-white border border-line rounded-2xl overflow-hidden">
      ...
      <div className="p-5 md:p-7">
        ...
        {draft.summary && <p className="mt-3 font-display text-fl-lg text-ink/80 leading-snug">{draft.summary}</p>}
        {draft.content ? (
  ...
  return (
    <div>
      ...
      {restore && (
        <div className="mt-4 rounded-xl border border-brass/40 bg-brass/10 px-4 py-3 text-fl-sm flex flex-wrap items-center justify-between gap-3">
          ...
          <span className="flex gap-2">
            ...
            <button type="button" onClick={() => { clearLocalDraft(draft.id); setRestore(null) }} className="px-3 py-1.5 rounded-md border border-line text-[11px] font-semibold uppercase tracking-[.12em] text-slate">Discard</button>
          </span>
        </div>
      )}
        ...
        <div className={`space-y-5 ${mobileTab === 'preview' ? 'hidden lg:block' : ''}`}>
          <Card title="Story">
            ...
            <textarea
              ...
              className="mt-3 w-full bg-transparent border-0 border-b border-line focus:border-steel outline-none py-2 font-display text-fl-lg leading-snug placeholder:text-fog/50 placeholder:font-body placeholder:text-fl-sm resize-none"
            />
            ...
            <div className="mt-4 flex flex-wrap items-center gap-1.5 text-[11px] font-semibold text-slate">
              {([['h2', 'H2'], ['bold', 'B'], ['italic', 'I'], ['bullet', '• List'], ['number', '1. List'], ['quote', '“ Quote'], ['link', 'Link']] as const).map(([k, lbl]) => (
                ...
                <ul className="mt-3 divide-y divide-line border-y border-line text-fl-sm">
                  {draft.attachments.map((a, i) => (
                    <li key={a.url} className="flex items-center gap-3 py-2">
                      ...
                      <button type="button" onClick={() => onChange(d => ({ ...d, attachments: d.attachments.filter((_, x) => x !== i) }))} className="text-red-700 text-[11px] font-semibold uppercase tracking-[.12em]">remove</button>
                    </li>
                  ))}
                </ul>
              )}
